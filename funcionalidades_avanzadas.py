...
class GestorFuncionalidadesAvanzadas:
    ...
    def __init__(self, db_path: str='data/gym_database.db'):
        ...
        self.worker_thread = threading.Thread(target=self._procesar_cola,
            daemon=True)
    ...
    def _inicializar_tablas(self):
        ...
        cursor.execute(
            'CREATE INDEX IF NOT EXISTS idx_fotos_user ON fotos_progreso(user_id)'
            )
        cursor.execute(
            'CREATE INDEX IF NOT EXISTS idx_objetivos_user ON objetivos(user_id)'
            )
        cursor.execute(
            'CREATE INDEX IF NOT EXISTS idx_logros_user ON logros(user_id)')
        cursor.execute(
            'CREATE INDEX IF NOT EXISTS idx_mensajes_dest ON mensajes_enriquecidos(destinatario_id)'
            )
        cursor.execute(
            'CREATE INDEX IF NOT EXISTS idx_analisis_user ON analisis_rendimiento(user_id)'
            )
        cursor.execute(
            'CREATE INDEX IF NOT EXISTS idx_recuperacion_user ON metricas_recuperacion(user_id)'
            )
        conn.commit()
        conn.close()

    def agregar_foto_progreso(self, user_id: int, imagen_path: str, peso_kg:
        float, notas: str='', medidas: Optional[Dict[str, float]]=None) ->int:
    ...
    def obtener_linea_tiempo_progreso(self, user_id: int, limite: int=50
        ) ->List[ProgressPhoto]:
        conn = self._get_connection()
        cursor = conn.cursor()
        cursor.execute(
        ...
        for row in cursor.fetchall():
            medidas = json.loads(row['medidas_json']) if row['medidas_json'
                ] else {}
            foto = ProgressPhoto(id=row['id'], user_id=row['user_id'],
                fecha=row['fecha'], imagen_base64=row['imagen_base64'],
                peso_kg=row['peso_kg'], notas=row['notas'], medidas=medidas)
            fotos.append(foto)
        conn.close()
        return fotos

    def comparar_fotos_progreso(self, user_id: int, foto1_id: int, foto2_id:
        int) ->Dict:
        conn = self._get_connection()
        cursor = conn.cursor()
        ...
        if not foto1 or not foto2:
            return {}
        medidas1 = json.loads(foto1['medidas_json']) if foto1['medidas_json'
            ] else {}
        medidas2 = json.loads(foto2['medidas_json']) if foto2['medidas_json'
            ] else {}
        cambios = {'peso_kg': foto2['peso_kg'] - foto1['peso_kg'],
            'dias_transcurridos': (datetime.fromisoformat(foto2['fecha']) -
            datetime.fromisoformat(foto1['fecha'])).days, 'cambios_medidas': {}
            }
    ...
    def crear_objetivo(self, user_id: int, nombre: str, descripcion: str,
        tipo: str, valor_objetivo: float, unidad: str, fecha_objetivo:
        Optional[str]=None, valor_actual: float=0) ->int:
        fecha_inicio = datetime.now().isoformat()
        hitos = []
        for pct in [25, 50, 75, 100]:
            hitos.append({'porcentaje': pct, 'valor': valor_actual + (
                valor_objetivo - valor_actual) * (pct / 100), 'alcanzado': 
                False, 'fecha_alcanzado': None})
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
            , (user_id, nombre, descripcion, tipo, valor_actual,
            valor_objetivo, unidad, fecha_inicio, fecha_objetivo, hitos_json))
        objetivo_id = cursor.lastrowid
        conn.commit()
        conn.close()
        return objetivo_id

    def actualizar_progreso_objetivo(self, objetivo_id: int, nuevo_valor: float
        ) ->Dict:
        conn = self._get_connection()
        cursor = conn.cursor()
        ...
        hitos = json.loads(objetivo['hitos_json']) if objetivo['hitos_json'
            ] else []
        for hito in hitos:
        ...
        return {'progreso_pct': progreso_pct, 'completado': completado,
            'hitos_alcanzados': [h for h in hitos if h['alcanzado']]}

    def obtener_objetivos(self, user_id: int, incluir_completados: bool=False
        ) ->List[Objetivo]:
        conn = self._get_connection()
        cursor = conn.cursor()
        ...
        for row in cursor.fetchall():
            ...
            objetivo = Objetivo(id=row['id'], user_id=row['user_id'],
                nombre=row['nombre'], descripcion=row['descripcion'], tipo=
                row['tipo'], valor_actual=row['valor_actual'],
                valor_objetivo=row['valor_objetivo'], unidad=row['unidad'],
                fecha_inicio=row['fecha_inicio'], fecha_objetivo=row[
                'fecha_objetivo'], progreso_pct=row['progreso_pct'],
                completado=bool(row['completado']), hitos=hitos)
    ...
    def _verificar_logros_progreso(self, user_id: int):
        ...
        cursor = conn.cursor()
        cursor.execute(
            'SELECT COUNT(*) as total FROM fotos_progreso WHERE user_id = ?',
            (user_id,))
        total_fotos = cursor.fetchone()['total']
        if total_fotos >= 1:
            self._otorgar_logro(user_id, 'Primera Foto',
                'Has registrado tu primera foto de progreso', 'progreso', '📸')
        if total_fotos >= 10:
            self._otorgar_logro(user_id, 'Documentador Dedicado',
                'Has registrado 10 fotos de progreso', 'progreso', '📷', 2)
        if total_fotos >= 50:
            self._otorgar_logro(user_id, 'Maestro del Progreso',
                'Has registrado 50 fotos de progreso', 'progreso', '🎥', 3)
        conn.close()
    ...
    def obtener_logros(self, user_id: int) ->List[Logro]:
        ...
        logros = []
        for row in cursor.fetchall():
            logro = Logro(id=row['id'], user_id=row['user_id'], nombre=row[
                'nombre'], descripcion=row['descripcion'], icono=row[
                'icono'], categoria=row['categoria'], fecha_obtenido=row[
                'fecha_obtenido'], nivel=row['nivel'])
            logros.append(logro)
        conn.close()
        return logros

    def enviar_mensaje_enriquecido(self, remitente_id: int, destinatario_id:
        int, tipo: str, contenido: str, archivo_path: Optional[str]=None
        ) ->int:
        archivo_base64 = None
        nombre_archivo = None
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """
            , (remitente_id, destinatario_id, tipo, contenido,
            archivo_base64, nombre_archivo, fecha))
        mensaje_id = cursor.lastrowid
        conn.commit()
        conn.close()
        return mensaje_id

    def obtener_mensajes_enriquecidos(self, user_id: int, limite: int=50
        ) ->List[MensajeEnriquecido]:
        conn = self._get_connection()
        cursor = conn.cursor()
        cursor.execute(
            """
        ...
        mensajes = []
        for row in cursor.fetchall():
            mensaje = MensajeEnriquecido(id=row['id'], remitente_id=row[
                'remitente_id'], destinatario_id=row['destinatario_id'],
                tipo=row['tipo'], contenido=row['contenido'],
                archivo_base64=row['archivo_base64'], nombre_archivo=row[
                'nombre_archivo'], leido=bool(row['leido']), fecha=row['fecha']
                )
            mensajes.append(mensaje)
        conn.close()
        return mensajes

    def registrar_sesion_entrenamiento(self, user_id: int, tipo_ejercicio:
        str, repeticiones: int, peso_kg: float, duracion_seg: int, **kwargs):
        fecha = datetime.now().isoformat()
        conn = self._get_connection()
        cursor = conn.cursor()
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
            , (user_id, fecha, tipo_ejercicio, repeticiones, peso_kg,
            duracion_seg, kwargs.get('frecuencia_cardiaca', None), kwargs.
            get('calorias', None), kwargs.get('calidad_forma', None),
            kwargs.get('notas', '')))
        conn.commit()
        conn.close()

    def registrar_metricas_recuperacion(self, user_id: int, calidad_sueno:
        int, horas_sueno: float, nivel_estres: int, **kwargs):
        fecha = datetime.now().isoformat()
        conn = self._get_connection()
        cursor = conn.cursor()
        cursor.execute(
        ...
        """
            , (user_id, fecha, calidad_sueno, horas_sueno, nivel_estres,
            kwargs.get('dolor_muscular', None), kwargs.get('fc_reposo',
            None), kwargs.get('variabilidad_fc', None), kwargs.get(
            'hidratacion_ml', None)))
        conn.commit()
        conn.close()

    def obtener_analisis_rendimiento(self, user_id: int, dias: int=30) ->Dict:
        ...
        ejercicios = {}
        for row in cursor.fetchall():
            ejercicios[row['tipo_ejercicio']] = {'peso_promedio': row[
                'peso_promedio'], 'reps_promedio': row['reps_promedio'],
        ...
        conn.close()
        return {'periodo_dias': dias, 'ejercicios': ejercicios,
            'recuperacion': {'calidad_sueno_promedio': recuperacion[
            'sueno_promedio'], 'horas_sueno_promedio': recuperacion[
            'horas_promedio'], 'nivel_estres_promedio': recuperacion[
            'estres_promedio']}}
